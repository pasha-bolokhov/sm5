...
\cite{Ted1,Moore:2001bv,CG,GK}. Finally, the 
...
\section{Conclusion}
	...
	QED admits a plenty of LV interactions,
	parametrized by background vectors and tensors.
	Some of these operators have been studied before
\cite{MP:,Gagnon:2004xh}.
	...
	In general, we use QED as a toy model, with which one can practice in
	building Lorentz-violating extensions. 

	The accent of this paper was mainly put on the Standard Model 
	domain.
	One way of writing down a LV extension of the Standard Model is to 
	list all possible interactions satisfying our criteria given in the Introduction.
	...
	sector is still very simple.
	However, one quickly runs into the problem of dimensional transmutation into 
	dimension three operators via quadratic divergencies.
	Even if one places the cut-off of the theory at the level of $ 1~\GeV $,
	the dangerous dimension five interactions would have to be tuned to be 
	(much) smaller than $ 10^{-31}~\GeV^{-1} $. 
	One therefore is naturally lead to a requirement of absence of uncontrollable divergencies
	in a consistent effective theory, by sweeping out the problematic operators
	by hand. 
	Another essential point is that 
	phenomenologically important is the effective theory below the EW scale.
	All interactions written down for the full Standard Model contribute 
	to the low energy theory operators written with only $ \suc $ and 
	$ {\rm U}_\el(1) $ gauge groups. 
	Constraining the low energy theory, one therefore automatically puts limits on
	the interactions existing above the EW scale. 
	
	The operators of the effective low-energy theory can be divided in two
	...
	All the other operators present in our effective theory do not affect
	propagation of particles directly, and we call them soft LV interactions.
	In general strong astrophysical constraints are not applicable to such
	operators. 
	Limits of the order $ 10^{-28-30}~\GeV^{-1} $ on them come from various types of 
	laboratory experiments, and are still sufficiently tight. 
	Finally, we separate out a unique lepton-number violating operator which only
	exists in the neutrino sector.
	Neutrino physics places a limit of $ 10^{-23-24}~\GeV^{-1} $ on this operator. 	
