...
\documentclass[letter,12pt]{revtex4}
...
\section{Introduction}
	...
	Most of developments consider only a sufficiently limited set
	...
	dimension three and four, but the domain of dimension five terms has
	...
	starting from Quantum mechanical ones, nuclear, high energy physics,
	astrophysical ones and reaching towards cosmology and gravity
	[{\it lots of refs}].
...
%	described had not been detected so far.
	...
	QED is a good and easy example to illustrate how LV can be 
	introduced into a theory and how to systematically parametrize 
	the LV interactions.
...
$ \widetilde{F}_{\mu\nu} $.
	...
	content,
	with some indices contracted and some being left external, we will
	quickly realize that many of them vanish. 
...
\end{equation}
	...
	on the equations of motion, and, in accord with the requirements
	given in [{\it Myers, Pospelov}], have to be ignored. 
...
$ F^{\mu\nu} \partial^\lambda F^{\rho\sigma} $,
	...
	It was shown that this operator has to be contracted with an
...
\end{equation}
	...
	we still want our 3-rank operator to be completely independent
	of them.
...
$ F^{\mu\nu} \partial^\lambda F^{\rho\sigma} $,
	...
	turns out to be the operator $ C^{\mu\nu\rho} $:
...
\end{eqnarray}
	...
	nevertheless we have listed them in square brackets for the future use 
	in the SM where they become nontrivial.
...
\subsection{1-loop RG coefficients}
	...
	only C-parity argument is obvious to apply.
	...
	The latter one is a rank three absolutely symmetric tensor, even
...
\[
	...
	~.
\]
	Close analysis reveals that the only operators that are subject
	to this effect are $ \wt{c}_1^\mu $ and $ D_1^{\mu\nu\rho} $
	which in this way generate extra corrections to themselves.

	{\bf
	The problem with the $ \wt{c}_1^\mu $ operator.
	}
...
\section{Classification of Operators of Dimension V in the Standard Model}
	...
	Since the LV physics governs at the UV, all LV operators must 
...
%	\otimes {\rm U}(1) $.
	...
	Though, there is one simplification arising from chirality of
	SM spinors (resulting in a less number of types of operators in the 
	QCD sector).

	Like in the previous section, LV interactions must obey 
...
\nonumber
	...
	~.
\end{eqnarray}
	The simplification of \eqref{SM_lepton} with respect to 
	\eqref{SM_quark} follows from the fact that leptons do not
	interact strongly.
...
\subsection{Higgs sector}	
	...
	LV also is present in the Higgs sector.
...
\end{eqnarray*}
	...
	Higgs and fermions. 
	This part is somewhat bigger and includes higher-rank operators.
...
\notag
	...
	\text{h.c.}
\end{align}
	where $ \epsilon H^* $ is the charge conjugate of Higgs.	
	One also has the same set of operators for interaction of Higgs with 
	leptons:
%%
...
\end{align}
	...
	energies only induce effectively higher-dimensional operators,
	and a such are not very interesting from phenomenological point of
	view.
	Instead we have only concentrated on the rank three absolutely
	symmetric operators $ C_X^{\mu\nu\rho} $ 
	and $ \kappa^{\mu\nu\rho} $.
	We chose them as more important, since these are the only operators
...
\section{Phenomenological Discussion}
	...
	We can now ignore the chirality of the particles, break the
...
$ C^{\mu\nu\rho} $
	emerges as a linear combination of the $ \ue $ and $ \sul $ 
	tensors:
\[
	C^{\mu\nu\rho}\Bigr|_{M_W} ~=~ 
...
\newpage
\section{Young tableaux and irreducible tensors}
